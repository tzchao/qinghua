...
class Dispatcher {
    ...
    static public function dispatch() {
        ...
        if(isset($_GET[$varPath])) { // 判断URL里面是否有兼容模式参数
            $varPath =I('get."$varPath"','','htmlspecialchars,trim');
            $PATH_INFO =I('server.PATH_INFO','','htmlspecialchars,trim');
            $PATH_INFO = $varPath;
            unset($varPath);
        }elseif(IS_CLI){ // CLI模式下 index.php module/controller/action/params/...
            $argv =I('server.argv','','htmlspecialchars,trim');
            
            $PATH_INFO = isset($argv[1]) ? $argv[1] : '';
        }
        ...
        if(C('APP_SUB_DOMAIN_DEPLOY')) {
            ...
            $HTTP_HOST =I('server.HTTP_HOST','','htmlspecialchars,trim');
                define('APP_DOMAIN',$HTTP_HOST); // 当前完整域名
                $rule = $rules[APP_DOMAIN];
            }else{
            $HTTP_HOST =I('server.HTTP_HOST','','htmlspecialchars,trim');

                if(strpos(C('APP_DOMAIN_SUFFIX'),'.')){ // com.cn net.cn 
                    $domain = array_slice(explode('.', $HTTP_HOST), 0, -3);
                }else{
                    $domain = array_slice(explode('.', $HTTP_HOST), 0, -2);                    
                }
                if(!empty($domain)) {
