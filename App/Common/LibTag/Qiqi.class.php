 ...
 * ----------------------------------------------------
 * $Author: luffy $
 * $Id: Qiqi.class.php 2016-06-14 10:38 by luffy $
 * ----------------------------------------------------
 */
...
<?php
	...
	if (!empty(\$_tablename) && \$_tablename != 'page') {
		...
		\$_list = D2('ArcView',"\$_tablename")->field("$field")->nofield('content,pictureurls')->where(\$where)->order("$orderby")->limit(\$limit)->select();
		if (empty(\$_list)) {
			\$_list = array();
		}
	}else {
...
str;
	...
	public function _email($attr,$content){
		return C('CFG_WEBEMAIL');
	}

	public function _logo($attr,$content){
		return C('CFG_WEBLOGO');
	}
