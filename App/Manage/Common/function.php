...
function get_element_html($name,$typeid, $tvalue = '', $vaule = '') {
	...
	if (!empty($tvalue)) {
		$array = explode("\n", str_replace("\r\n", "\n", trim($tvalue,"\r\n")));
		if (in_array($array[0], array('select','radio','checkbox','text','textarea','upload'))) {
			$type = $array[0]; 
			unset($array[0]);
			if(strpos($tvalue,'|')){
				$tvalue  = array();
	...
	switch ($type) {
		...
		case 'select':
			...
			break;
		case 'upload':
			$urlUpload =  U('Public/upload', array('tb' => 2));
			$urlPicture = U('Public/browseFile', array('stype' => 'picture'));
			$str .= <<<EOF
		<script>$(function () {
					//缩略图上传
					var litpic_tip = $(".litpic_tip");
					var btn = $(".litpic_btn span");
					$("#fileupload").wrap("<form id='myupload' action='{$urlUpload}' method='post' enctype='multipart/form-data'></form>");
						...
						$("#myupload").ajaxSubmit({
							...
							success: function(data) {
								if(data.state == 'SUCCESS'){					
									...
									$('#litpic_show').html("<img src='"+timg+"' width='auto'>");
									$("#litpic").val(img);
								}else {
				...
				function selectPicture(sfile) {
					...
					$.jBox.close(true);
					$("#litpic").val(sfile);
					$('#litpic_show').html("<img src='"+sfile+"' width='auto'>");
				}
							</script>
EOF;
			$str .= '<div class="litpic_show"><div style="float:left;"> <input type="text" class="inp_large" name="'.$name.'" id="litpic"  value="'.$vaule.'" />';
			$str .= '<input type="button" class="btn_blue_b" id="BrowerPicture" value="选择站内图片"> </div><div class="litpic_btn"> <span>添加图片</span><input id="fileupload" type="file" name="mypic">';			
			$str .= '</div><div class="litpic_tip"></div> <div id="litpic_show">';					   
			if(!empty($vaule)){
				$str .= '<img src="'.$vaule.'" width="auto" />';
			}
			$str .= '</div></div>';
		break;
		default:
 ...
 * @return null
 */
function addSearch($data,$id,$modelid){
	$data['itemid'] = intval($id);
	$data['modelid'] = intval($modelid);
	M('Search')->add($data);
}
 ...
 * @return null
 */
function editSearch($data,$id,$cid,$modelid){
	$where = "itemid = $id AND cid = $cid";
	$data['modelid'] = intval($modelid);
	unset($data['id']);
	M('Search')->where($where)->save($data);
}
 ...
 * @return null
 */
function moveSearch($id,$cid,$modelid){
	$where = array('itemid'=>array('in',$id),'modelid' => $modelid);
	M('Search')->where($where)->setField('cid',$cid);
}

/**
 * 删除文章
 */
function delSearch($id,$modelid,$status = 1){
	$id = intval($id);
	$cid = intval($cid);
	$where = "itemid = $id AND modelid = $modelid";
	M('Search')->where($where)->setField('status',$status);
}

/**
 * 批量删除
 */
function delBatchSearch($idArr,$modelid,$status = 1){
	$where = array(
			'itemid'=>array('in',$idArr),
			'modelid'=>$modelid,
	);
	M('Search')->where($where)->setField('status',$status);
}
