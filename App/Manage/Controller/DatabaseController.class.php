...
class DatabaseController extends CommonController {
    ...
    public function backup() {
        ...
        $time = time();//开始时间
        $path = $this->getDbPath() . "/qiqicmstables_" . date("Ymd") . "_" . get_randomstr(5);
    ...
    public function downFile() {
        $file = I('get.file','','htmlspecialchars,trim');
        ...
        $filePath = $path[$type] . $file;               //需修改
        if (!file_exists($filePath)) {
            $this->error("该文件不存在，可能是被删除");
        }
        $filename = basename($filePath);
        header("Content-type: application/octet-stream");
        header('Content-Disposition: attachment; filename="' . $filename . '"');//需修改
