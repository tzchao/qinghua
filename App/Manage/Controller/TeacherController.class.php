...
class TeacherController extends CommonContentController {
	...
	public function index() {		
		...
		if ($pid) {
			...
			$where = array('teacher.status' => 0, 'cid' => array('in', $idarr));
		}else {
			$where = array('teacher.status' => 0);
		}

		if (!empty($keyword)) {
			$where['teacher.title'] = array('LIKE', "%{$keyword}%");
		}
		
		
		//import('Class.Page', APP_PATH.'Lib/');
		$count = D2('ArcView','teacher')->where($where)->count();

		$page = new \Common\Lib\Page($count, 10);		
		$page->rollPage = 7;
		$page->setConfig('theme','%HEADER% %FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END%');
		$limit = $page->firstRow. ',' .$page->listRows;
		$art = D2('ArcView','teacher')->nofield('content')->where($where)->order('teacher.listorder ASC,teacher.id DESC')->limit($limit)->select();
		$this->assign('pid', $pid);
	...
	public function addPost() {
		...
		if($id = M('teacher')->add($data)) {
			...
			if (!empty($pic)) {				
				...
				M('teacher')->save($updata);
			}

			//attachment index入库
			insert_att_index($attid_arr,$id,$modelid);
			//填加成功后加到搜索表里
			addSearch($data,$id,$modelid);
			...
			$this->success('添加文章成功',U('Teacher/index', array('pid' => $pid)));
		}else {
			$this->error('添加文章失败');
		}
	}
	...
	public function editPost() {
		...
		if (false !== M('teacher')->save($data)) {
			...
			//attachment index入库
			insert_att_index($attid_arr,$id,$modelid);
			editSearch($data,$id,$data['cid'],$modelid);
			...
			$this->success('修改成功', U('Teacher/index', array('pid' => $pid)));
		}else {
	...
	public function move() {
		...
		if (IS_POST) {
			...
			if (false !== M($actionName)->where(array('id'=> array('in', $id)))->setField('cid',$cid)) {
				...
				moveSearch($id,$cid,$modelid);
				
				$this->success('移动成功', U('Teacher/index', array('pid' => $pid)));
			}else {
				$this->error('移动失败');
			}			
			exit();
	...
	public function trach() {
		...
		$where = array('teacher.status' => 1);
		$count = D2('ArcView','teacher')->where($where)->count();
		$page = new \Common\Lib\Page($count, 10);
		$page->rollPage = 7;
		$page->setConfig('theme','%HEADER% %FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END%');
		$limit = $page->firstRow. ',' .$page->listRows;
		$art = D2('ArcView','teacher')->nofield('content')->where($where)->limit($limit)->select();
	...
	public function del() {
		...
		if (false !== M('teacher')->where(array('id' => $id))->setField('status', 1)) {

			del_cache_html('Show/index_*_'. $id.'.', false, 'show:index');			
			
			//删除成功后更改SEARCH
			//获取属于分类信息,得到modelid
			$selfCate = Category::getSelf(get_category(0), $pid);//当前栏目信息
			$modelid = $selfCate['modelid'];
			
			delSearch($id,$modelid,1);
			$this->success('删除成功', U('Teacher/index', array('pid' => $pid)));
	...
	public function delBatch() {
		...
		if (false !== M('teacher')->where(array('id' => array('in', $idArr)))->setField('status', 1)) {
			
			//更新静态缓存
			foreach ($idArr as $v) {
				del_cache_html('Show/index_*_'. $v.'.', false, 'show:index');	
			}
			//. M('teacher')->getlastsql();
			
			//删除成功后更改SEARCH
			//获取属于分类信息,得到modelid
			$selfCate = Category::getSelf(get_category(0), $pid);//当前栏目信息
			$modelid = $selfCate['modelid'];
			delBatchSearch($idArr,$modelid,1);
			$this->success('批量删除成功', U('Teacher/index', array('pid' => $pid)));
	...
	public function restore() {
		...
		if (false !== M('teacher')->where(array('id' => $id))->setField('status', 0)) {
			
			//删除成功后更改SEARCH
			delSearch($id,$pid,0);
			$this->success('还原成功', U('Teacher/trach', array('pid' => $pid)));
	...
	public function restoreBatch() {
		...
		if (false !== M('teacher')->where(array('id' => array('in', $idArr)))->setField('status', 0)) {
			
			//删除成功后更改SEARCH
			delBatchSearch($idArr,$pid,0);
			$this->success('还原成功', U('Teacher/trach', array('pid' => $pid)));
	...
	public function clear() {
		...
		$pid = I('get.pid', 0, 'intval');
		$modelid = D2('ArcView','teacher')->where(array('id' => $id))->getField('modelid');
		...
		if (M('teacher')->delete($id)) {
			...
			$this->success('彻底删除成功', U('Teacher/trach', array('pid' => $pid)));
		}else {
			$this->error('彻底删除失败');
		}
	}
	...
	public function clearBatch() {
		...
		$modelid = D2('ArcView','teacher')->where(array('id' => $idArr[0]))->getField('modelid');//
	...
	public function listorder(){
		if(count($_POST['listorder'])){
			foreach ($_POST['listorder'] as $key => $v){
				$data['id'] = $key;
				$data['listorder'] = $v;
				M('teacher')->save($data);
			}
