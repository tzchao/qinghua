...
class ArticleController extends CommonContentController {
	...
	public function addPost() {
		...
		//非管理员发布的都加上待审核
		$examine = session('yang_adm_roleid');
		if($examine<2){
			$data['examine'] = 0;
		}
		else{
	...
	public function editPost() {
		...
		$data['examine'] = $data['examine'] == '2' ? 1 : $data['examine'];
	...
	public function examine(){
		$examine = I('examine',0,'intval');
		$id = I('id',0,'intval');
		if(false !== M('article')->where(array('id'=>$id))->setField('examine',$examine)){
			$this->success('审核操作成功');
		}
		else{
			$this->error('审核操作失败');
