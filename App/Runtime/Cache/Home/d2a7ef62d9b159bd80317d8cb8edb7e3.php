<?php if (!defined('THINK_PATH')) exit();?><!-- 新闻资讯 -->
...
<nav class="navbar">
 ...
 $_typeid = intval(0); $_type = "son"; $_temp = explode(',', "10"); $_temp[0] = $_temp[0] > 0? $_temp[0] : 10; if (isset($_temp[1]) && intval($_temp[1]) > 0) { $_limit[0] = $_temp[0]; $_limit[1] = intval($_temp[1]); }else { $_limit[0] = 0; $_limit[1] = $_temp[0]; } if($_typeid == -1) $_typeid = I('cid', 0, 'intval'); $__catlist = get_category(1); if (0) { $__catlist = Common\Lib\Category::getLevelOfModelId($__catlist, 0); } if (1 == 0) { $__catlist = Common\Lib\Category::clearPageAndLink($__catlist); } if($_typeid == 0 || $_type == 'top') { $_catlist = Common\Lib\Category::toLayer($__catlist); }else { if ($_type == 'self') { $_typeinfo = Common\Lib\Category::getSelf($__catlist, $_typeid ); $_catlist = Common\Lib\Category::toLayer($__catlist, 'child', $_typeinfo['pid']); }else { $_catlist = Common\Lib\Category::toLayer($__catlist, 'child', $_typeid); } } foreach($_catlist as $autoindex => $catlist): if($autoindex < $_limit[0]) continue; if($autoindex >= ($_limit[1]+$_limit[0])) break; $catlist['url'] = get_url($catlist); if(count($catlist['child'][0]['child']) == 0): ?><li class="erji-li">
	...
	<div class="ny-banner">
		<?php
 $type = Common\Lib\Category::getSelf(get_category(0), $pid); $type['url'] = get_url($type); ?><img src="<?php echo ($type["catpic"]); ?>" class="img-responsive" alt="<?php echo ($type["name"]); ?>">
	</div>
	<!-- 内容主体 -->
 ...
 $type = Common\Lib\Category::getSelf(get_category(0), $cid); $type['url'] = get_url($type); ?><span class="<?php if(($type['pid']) == $catlist['id']): ?>cur<?php endif; ?>"><i></i><a href="javascript:;"><?php echo ($catlist["name"]); ?></a></span>
    ...
    <div class="column-tit">
		<i></i><?php echo ($cate["name"]); ?>
	</div>
	<div class="news-project">
      <ul>
      <?php
 $_typeid = -1; $_keyword = ""; if($_typeid == -1) $_typeid = I('cid', 0, 'intval'); if ($_typeid>0 || substr($_typeid,0,1) == '$') { $_selfcate = Common\Lib\Category::getSelf(get_category(), $_typeid); $_tablename = strtolower($_selfcate['tablename']); $ids = Common\Lib\Category::getChildsId(get_category(), $_typeid, true); $where = array($_tablename.'.status' => 0,$_tablename.'.examine' => 0, $_tablename .'.cid'=> array('IN',$ids)); }else { $_tablename = 'article'; $where = array($_tablename.'.status' => 0, $_tablename.'.examine' => 0); } if ($_keyword != '') { $where[$_tablename.'.title'] = array('like','%'.$_keyword.'%'); } if (0 > 0) { $where['_string'] = $_tablename.'.flag & 0 = 0 '; } if (!empty($_tablename) && $_tablename != 'page') { if (20 > 0) { $count = D2('ArcView',"$_tablename")->where($where)->count(); $ename = I('e', '', 'htmlspecialchars,trim'); if (!empty($ename) && C('URL_ROUTER_ON') == true) { $param['p'] = I('p', 1, 'intval'); $param_action = '/'.$ename; }else { $param = array(); $param_action = ''; } $thisPage = new \Common\Lib\Page($count, 20, $param, $param_action); $thisPage->rollPage = 5; $thisPage->setConfig('theme'," %FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END%"); $limit = $thisPage->firstRow. ',' .$thisPage->listRows; $page = $thisPage->show(); }else { $limit = "10"; } $_list = D2('ArcView',"$_tablename")->field("")->nofield('content,pictureurls')->where($where)->order("publishtime DESC")->limit($limit)->select(); if (empty($_list)) { $_list = array(); } }else { $_list = array(); } foreach($_list as $autoindex => $list): $_jumpflag = ($list['flag'] & B_JUMP) == B_JUMP? true : false; $list['url'] = get_content_url($list['id'], $list['cid'], $list['ename'], $_jumpflag, $list['jumpurl']); if(0) $list['title'] = str2sub($list['title'], 0, 0); if(0) $list['description'] = str2sub($list['description'], 0, 0); ?><li><a href="<?php echo ($list["url"]); ?>"><i></i><h1 title="<?php echo ($list["title"]); ?>" style="color:<?php echo ($list["color"]); ?>"><?php echo ($list["title"]); ?></h1><span><?php echo (date('Y-m-d',$list["publishtime"])); ?></span></a></li><?php endforeach;?>
      </ul>
      <?php echo ($page); ?>
    </div>
  </div>
