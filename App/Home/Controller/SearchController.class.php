...
class SearchController extends HomeCommonController{
	...
	public function index(){
		...
		$type = I('type','all','htmlspecialchars,trim');
		if($type == 'all'){
			//搜索全站
			if($keyword == '请输入关键词') $keyword = '';
			if(!empty($keyword)){
				$Search = M('Search');
				$where = "(title LIKE '%$keyword%' OR keywords LIKE '%$keyword%' OR description Like '%$keyword%' OR shorttitle like '%$keyword%') AND status = 0";
				$count = $Search->where($where)->count();
				...
				$vlist = $Search->where($where)->order('id desc')->limit($limit)->select();
			}
